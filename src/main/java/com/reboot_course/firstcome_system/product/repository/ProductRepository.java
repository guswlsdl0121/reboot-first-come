...
public interface ProductRepository extends JpaRepository<Product, Integer> {
    @Query("""
            SELECT new com.reboot_course.firstcome_system.product.dto.response.ProductMain(p.id, p.name, p.price)
            FROM Product p
            WHERE p.id < :cursor
            ORDER BY p.id DESC
            LIMIT :size
            """)
    List<ProductMain> getProducts(@Param("size") Integer size, @Param("cursor") Integer cursor);
}
