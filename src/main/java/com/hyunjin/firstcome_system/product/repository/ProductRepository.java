package com.hyunjin.firstcome_system.product.repository;
...
public interface ProductRepository extends JpaRepository<Product, Integer> {
    @Query("""
            SELECT new com.hyunjin.firstcome_system.product.dto.response.ProductItemDTO(p.id, p.name, p.price)
            FROM Product p
            WHERE p.id < :cursor
            ORDER BY p.id DESC
            LIMIT :size
